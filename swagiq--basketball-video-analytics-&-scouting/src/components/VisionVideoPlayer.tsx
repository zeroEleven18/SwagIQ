...
export const VisionVideoPlayer: React.FC<VisionVideoPlayerProps> = ({
  ...
  // 1. Initial Duration calculation based on game data
  const initialDuration = useMemo(() => {
    ...
    if (game.shots && game.shots.length > 0) {
      ...
      if (maxShot > 360) {
        return Math.max(2400, maxShot + 60);
      }
    }
    return 2400; // 40:00 FIBA match default
  }, [game.videoDurationSec, game.shots]);
  ...
  // Tracking status: 'recording' (collecting data), 'paused' (warm-up / halftime / timeout), 'stopped'
  const [trackingStatus, setTrackingStatus] = useState<'recording' | 'paused' | 'stopped'>('recording');
  const [isStopModalOpen, setIsStopModalOpen] = useState<boolean>(false);
  const [currentPeriodName, setCurrentPeriodName] = useState<string>('1° Quarto (Q1)');
  const [filterUntrackedEvents, setFilterUntrackedEvents] = useState<boolean>(true);
  
  // Official tracking periods (e.g. Q1, Q2, Q3, Q4)
  const [trackingPeriods, setTrackingPeriods] = useState<TrackingPeriod[]>(() => {
    ...
    // Default standard periods based on duration
    const dur = initialDuration;
    return [
      { id: 'tp-1', name: '1° Quarto (Q1)', startSec: 15, endSec: Math.floor(dur * 0.25) - 10, status: 'completed' },
      { id: 'tp-2', name: '2° Quarto (Q2)', startSec: Math.floor(dur * 0.25) + 20, endSec: Math.floor(dur * 0.5) - 10, status: 'completed' },
      { id: 'tp-3', name: '3° Quarto (Q3)', startSec: Math.floor(dur * 0.5) + 20, endSec: Math.floor(dur * 0.75) - 10, status: 'completed' },
      { id: 'tp-4', name: '4° Quarto (Q4)', startSec: Math.floor(dur * 0.75) + 20, endSec: dur - 10, status: 'completed' }
    ];
  });
  ...
  useEffect(() => {
    const startT = game.shots[0]?.videoTimestamp || 15;
    ...
    if (isYouTubeVideo) {
      sendYouTubeCommand('seekTo', [startT, true]);
      sendYouTubeCommand('pauseVideo');
    }
  }, [game.id, initialDuration]);
  ...
  }, [isPlaying, duration, playbackRate, isLocalOrDirectVideo]);
  ...
  // Standard Quarter Breakpoints based on total duration
  ...
  // ================= DYNAMIC SCORE & GAME CLOCK =================
  // Calculate live cumulative score up to currentTime based on visible shot events
  const calculatedHomeScore = visibleShots
    .filter((s) => s.teamId === 'home' && s.made && s.videoTimestamp <= currentTime)
    .reduce((acc, s) => acc + (s.points || (s.shotType === '3PT' ? 3 : 2)), 0) + (game.homeTeam.q1 || 20);

  const calculatedAwayScore = visibleShots
    .filter((s) => s.teamId === 'away' && s.made && s.videoTimestamp <= currentTime)
    .reduce((acc, s) => acc + (s.points || (s.shotType === '3PT' ? 3 : 2)), 0) + (game.awayTeam.q1 || 18);

  // Dynamic game clock estimation (countdown from 10:00 per quarter)
  const quarterIndex = Math.min(4, Math.max(1, Math.floor((currentTime / duration) * 4) + 1));
  const quarterProgress = (currentTime % (duration / 4)) / (duration / 4);
  const remainingQuarterSeconds = Math.max(0, Math.floor(600 * (1 - quarterProgress)));
  const clockMin = Math.floor(remainingQuarterSeconds / 60);
  const clockSec = remainingQuarterSeconds % 60;
  const liveGameClock = `${clockMin.toString().padStart(2, '0')}:${clockSec.toString().padStart(2, '0')} Q${quarterIndex}`;
  ...
  // Determine active tactical set based on time or closest tactic
  const activeTactic = game.tactics.find((t) => 
    t.videoTimestamps?.some((ts) => Math.abs(ts - currentTime) < 25)
  ) || game.tactics[0];
  ...
  return (
    <div className="space-y-4">
      {/* ================= TOP PANEL: SIMPLIFIED STATS & QUARTER COMMAND BAR ================= */}
      <div className="bg-slate-900/95 border border-slate-800 rounded-2xl p-3 sm:p-4 shadow-xl flex flex-wrap items-center justify-between gap-3">
        {/* Left: 3 Main Tracking Actions (INIZIA, PAUSA, STOP) with glowing active states */}
        <div className="flex items-center space-x-2">
          {/* INIZIA PRESA STATISTICHE */}
          <button
            onClick={handleStartTracking}
            className={`flex items-center space-x-2 px-4 py-2 rounded-xl text-xs font-black tracking-wide border transition-all transform active:scale-95 ${
              trackingStatus === 'recording'
                ? 'bg-emerald-600 border-emerald-400 text-white shadow-lg shadow-emerald-600/40 ring-2 ring-emerald-400/80 animate-none'
                : 'bg-slate-950 hover:bg-slate-800 border-slate-800 hover:border-emerald-500/50 text-emerald-400'
            }`}
            title="Avvia o riprendi la presa statistica e tracciamento sul video"
          >
            <span className={`w-2.5 h-2.5 rounded-full ${
              trackingStatus === 'recording' ? 'bg-white animate-ping' : 'bg-emerald-500'
            }`} />
          ...
          <button
            ...
            className={`flex items-center space-x-1.5 px-3.5 py-2 rounded-xl text-xs font-bold border transition-all transform active:scale-95 ${
              trackingStatus === 'paused'
                ? 'bg-amber-500 border-amber-300 text-slate-950 font-black shadow-lg shadow-amber-500/30 ring-2 ring-amber-400'
          ...
          <button
            ...
            className={`flex items-center space-x-1.5 px-3.5 py-2 rounded-xl text-xs font-bold border transition-all transform active:scale-95 ${
              trackingStatus === 'stopped'
                ? 'bg-red-600 border-red-400 text-white font-black shadow-lg shadow-red-600/30 ring-2 ring-red-400'
        ...
        {/* Right: Quarter Selectors (1° Quarto, 2° Quarto, 3° Quarto, 4° Quarto, Overtime) with bright glowing active highlight */}
          ...
          {[
            ...
            return (
              <button
                ...
                title={`Vai a inizio ${q.label} e avvia tracciamento`}
              >
                <span>{q.label}</span>
              </button>
            );
      ...
      {/* ================= TOP VIDEO TOOLBAR: OVERLAYS & SCOREBOARD ================= */}
      <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-3 flex flex-wrap items-center justify-between gap-3 shadow-md">
        <div className="flex items-center space-x-3">
          ...
          <div className="hidden sm:flex items-center space-x-1.5 bg-slate-950 px-3 py-1 rounded-xl border border-slate-800 text-xs">
            <span className="text-slate-400">Schema:</span>
            <span className="text-orange-400 font-semibold truncate max-w-[180px]">
              {activeTactic?.name || 'Pick & Roll Primario'}
            </span>
          </div>

          {/* Real-time Game Clock & Live Score Chip */}
          <div className="flex items-center space-x-2 bg-slate-950 px-3 py-1 rounded-xl border border-slate-800 font-mono text-xs">
            ...
            <span className="text-orange-400 font-bold ml-1 px-1.5 py-0.5 rounded bg-orange-500/10 border border-orange-500/20">
              {liveGameClock}
            </span>
          </div>
        </div>
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-lg text-[11px] font-semibold border transition-all ${
              showBBoxes 
                ...
                : 'bg-slate-800/60 text-slate-400 border-slate-700/60 opacity-60'
            }`}
          >
            Bounding Box
          </button>
      ...
      <div className="relative aspect-video w-full bg-slate-950 rounded-3xl overflow-hidden border border-slate-800 shadow-2xl group select-none">
        ...
        {isLocalOrDirectVideo && game.videoUrl ? (
          ...
          /* YouTube Embed Player with enablejsapi=1 */
          <div className="absolute inset-0 w-full h-full bg-black">
            <iframe
              ref={youtubeIframeRef}
              id="youtube-player-frame"
              src={getYouTubeEmbedUrl(effectiveYouTubeId, false)}
