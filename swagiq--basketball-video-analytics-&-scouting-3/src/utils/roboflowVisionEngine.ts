...
export function generateVisionMatchData(options: VisionMatchGeneratorOptions): BasketballGame {
  ...
  const effectiveDuration = videoDurationSec && videoDurationSec > 0 ? videoDurationSec : 0;
  ...
  // 3. Generate Shot Events (Only for sample demo matches without user video; for real YouTube/custom videos start clean)
  const shots: ShotEvent[] = [];
  const isCustomOrLiveVideo = Boolean(videoUrl) || Boolean(youtubeId) || videoSourceType === 'youtube' || videoSourceType === 'local' || videoSourceType === 'twitch';
  
  if (!isCustomOrLiveVideo && effectiveDuration > 0) {
    const totalShots = 28;
