...
export const TacticalAnalysis: React.FC<TacticalAnalysisProps> = ({
  ...
  const hasRecordedStats = (game.shots && game.shots.length > 0) || 
    (game.homeTeam.score > 0 || game.awayTeam.score > 0) ||
    (game.tacticalSchemes && game.tacticalSchemes.length > 0);

  if (!hasRecordedStats) {
    return (
      <div className="space-y-6 max-w-4xl mx-auto py-8">
        <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-8 sm:p-10 shadow-2xl text-center space-y-6">
          ...
          <div className="space-y-2 max-w-lg mx-auto">
            <h2 className="text-xl font-black text-white">Analisi Tattica in Attesa di Dati</h2>
            <p className="text-sm text-slate-400 leading-relaxed">
              La partita corrente non ha ancora registrato tiri o possessi tracciati (punteggio 0-0).
              L'estrazione degli schemi offensivi, difensivi, efficacia PPP e conformità SAM 3 si attiverà automaticamente durante la riproduzione del video o la registrazione dei tiri.
            </p>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 max-w-md mx-auto text-left space-y-2 text-xs text-slate-400">
            <div className="font-bold text-slate-300 flex items-center gap-1.5">
              ...
              <span>Cosa verrà estratto automaticamente:</span>
            </div>
            <ul className="space-y-1 pl-4 list-disc text-[11px] text-slate-400">
              <li>Schemi offensivi e difensivi più frequenti (Pick & Roll, Horns, 5-Out)</li>
              <li>Punti per Possesso (PPP) e % di successo reale</li>
              <li>Punteggio di conformità di posizionamento SAM 3</li>
            </ul>
          </div>
        </div>
      </div>
