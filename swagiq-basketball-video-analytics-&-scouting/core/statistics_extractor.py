...
class ShotEvent:
    ...
    timestamp: float  # secondi dal'inizio del video
    player_id: int
    player_name: str
    shot_type: ShotType
    result: ShotResult
...
class PlayerStats:
    ...
    def calculate_per_minute_stats(self) -> Dict[str, float]:
        ...
        return {
            "ppg": self.points / self.minutes_played * 40,  # Punti per 40 minuti
            "rpg": self.total_rebounds / self.minutes_played * 40,
            "apg": self.assists / self.minutes_played * 40,
            "tov_per_game": self.turnovers / self.minutes_played * 40,
        }
...
class StatisticsExtractor:
    ...
    def __init__(self):
        ...
        self.processing_log: List[str] = field(default_factory=list)
    ...
    def initialize_game(self, home_team: str, away_team: str):
        ...
        logger.info(f"Game initialized: {home_team} vs {away_team}")
    
    def add_player_to_team(self, team: str, player_id: int, player_name: str, 
                          jersey_number: int):
        """Aggiunge un giocatore al roster della squadra"""
        player = PlayerStats(
            player_id=player_id,
        ...
        if team == self.team_home.team_name:
            self.team_home.players.append(player)
        elif team == self.team_away.team_name:
    ...
    def record_shot(self, shot_event: ShotEvent):
        """Registra un evento di tiro"""
        self.shot_events.append(shot_event)
        
        # Aggiorna le statistiche della squadra
        team = self.team_home if shot_event.team == self.team_home.team_name else self.team_away
        team.shots_attempted += 1
        
        if shot_event.result == ShotResult.MADE:
            team.shots_made += 1
            if shot_event.shot_type == ShotType.THREE_POINTER:
                team.points += 3
                team.three_pointers_made += 1
            elif shot_event.shot_type == ShotType.FREE_THROW:
                team.points += 1
        ...
        logger.info(f"Shot recorded: {shot_event.player_name} - {shot_event.result.value}")
    
    def _get_player(self, player_id: int, team: str) -> Optional[PlayerStats]:
        """Ottiene un giocatore dal roster della squadra"""
        team_obj = self.team_home if team == self.team_home.team_name else self.team_away
    ...
    def export_statistics(self, filepath: str):
        ...
        stats = {
            ...
            "game": {
                "home_team": self.team_home.to_dict(),
                "away_team": self.team_away.to_dict(),
            },
            "shot_events": [shot.to_dict() for shot in self.shot_events],
        }
...
if __name__ == "__main__":
    ...
    extractor.initialize_game("Lakers", "Celtics")
    
    # Aggiungi giocatori
    extractor.add_player_to_team("Lakers", 1, "LeBron James", 23)
    extractor.add_player_to_team("Lakers", 2, "Anthony Davis", 3)
    extractor.add_player_to_team("Celtics", 3, "Jayson Tatum", 0)
    extractor.add_player_to_team("Celtics", 4, "Jaylen Brown", 7)
    
    # Registra alcuni tiri
    shot1 = ShotEvent(
        timestamp=120.5,
        player_id=1,
    ...
    # Esporta statistiche
    extractor.export_statistics("game_stats.json")
